...
const CATEGORY_BY_NAME = {
  红烧鸡腿: '荤菜', 蒜蓉西兰花: '素菜', 杂粮米饭: '主食', 酱牛肉: '荤菜',
  清炒菠菜: '素菜', 牛肉拉面: '主食', 蒸蛋羹: '素菜', 蒸玉米: '主食',
};
const DISH_ALLERGENS = {
  红烧鸡腿: [], 蒜蓉西兰花: [], 杂粮米饭: ['麸质'], 酱牛肉: [],
  清炒菠菜: [], 牛肉拉面: ['麸质'], 蒸蛋羹: ['鸡蛋'], 蒸玉米: [],
};
const MEAL_FIT = {
  早餐: ['蒸玉米', '蒸蛋羹', '牛肉拉面', '杂粮米饭'],
  午餐: ['红烧鸡腿', '酱牛肉', '杂粮米饭', '蒜蓉西兰花', '清炒菠菜'],
  晚餐: ['清炒菠菜', '蒜蓉西兰花', '杂粮米饭', '蒸蛋羹', '酱牛肉'],
};
...
function buildDishes() {
  return Object.values(DATA.foodDB).map(function (f, i) {
    return {
      ...
      name: f.name,
      emoji: f.emoji,
      category: CATEGORY_BY_NAME[f.name] || '热菜',
      cal: f.cal,
      protein: f.protein,
      fat: f.fat,
      carbs: f.carbs,
